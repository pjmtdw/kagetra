<template>
  ...
  <div v-else class="day rounded p-2"
    ...
    <div>
      <span class="font-weight-bold">{{ day.date }}</span>
      <span v-if="mobile" class="font-weight-bold">({{ weekday_ja[day.day] }})</span>
    ...
    <template v-for="e in day.events">
      <div v-if="!isPublic || e.public" v-show="!bulkEditing && !holidayEditing" class="mt-1">
        {{ e.name }}
      </div>
    </template>
    <template v-for="it in day.items">
      <div v-if="!isPublic || it.public" v-show="!holidayEditing" :key="it.id" class="dayitem mt-1" :class="{ selected: it.id === selected.id, 'cursor-pointer': bulkEditing }" @click="select($event, it)">
        <span>{{ it.name }}</span>
