  ...
  namespace '/api/user' do
    ...
    get '/newly_message' do
      ...
      serialized_contest_kind = Event.serialization_map[:kind].call(:contest)
      {
        ...
        result_comment: Event.new_threads(@user,{done:true,kind: serialized_contest_kind}).map{|x|x.select_attr(:name,:id)},
        ev_done_comment: Event.new_threads(@user,Sequel.expr(done:true) & Sequel.~(kind:serialized_contest_kind)).map{|x|x.select_attr(:name,:id)},
